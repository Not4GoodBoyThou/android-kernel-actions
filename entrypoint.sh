...
if [[ $arch = "arm64" ]]; then
    ...
    if [[ $compiler = gcc/* ]]; then
        ...
        if $binutils; then
            ...
            make_opts="CC=clang"
            make_opts+=" LD=ld.lld NM=llvm-nm AR=llvm-ar STRIP=llvm-strip OBJCOPY=llvm-objcopy"
...
mv -v "$workdir"/"$vendor_path" "$workdir"/"$kernel_path"/vendor
cd "$workdir"/"$kernel_path" || exit 127
start_time="$(date +%s)"
date="$(date +%d%m%Y-%I%M)"
tag="$(git branch | sed 's/*\ //g')"
echo "branch/tag: $tag"
echo "make options:" $arch_opts $make_opts $host_make_opts $defconfig
...
if ! make O=out $arch_opts $make_opts $host_make_opts -j"$(nproc --all)"; then
    ...
    make V=1 $arch_opts $make_opts $host_make_opts mrproper

    make V=1 $arch_opts $make_opts $host_make_opts module_prepare

    make V=1 $arch_opts $make_opts $host_make_opts INSTALL_MOD_PATH=. INSTALL_MOD_STRIP=1 modules_install
